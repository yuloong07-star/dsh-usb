...
DeepSeek Harness（DSH）便携版，专为 U 盘 / 移动硬盘随身使用打造。双击安装包即解压即用，新机器无需安装任何环境。

> **本项目基于 [DSH-Desktop](https://github.com/anywhere-labs/deepseek-harness-desktop)（deepseek-harness-desktop）开发**，在其桌面端基础上改造为便携模式：运行数据全部保存在安装包旁侧目录，不写 `%APPDATA%`、不写注册表。
